...
KPM PARAMETERS
  ...
  -batch 
  		Sets, without any parameters if it is a batch KPM run.

  -K_batch {'integer','integer','integer'}
  		Defines the start, step, and max number of gene exceptions allowed, 
  		when doing a batch run with INES.
  		
  -LN_batch  {'integer','integer','integer'}
  		Defines the start, step, and max number of case (L) exceptions for matrix N.
  		
  -perturbation  {'integer','integer','integer','integer'}
  		Defines the start, step, and max degree of perturbation. 
		Last value is the amount of graphs per step.

  -perturbation_technique {'edgeremove' | 'edgerewire' | 'nodeswap' | 'noderemove'}
  		Defines the technique used to perform the perturbation in a KPM run.

  -validation_file
  		The path to the validation files, for comparing with fx gold standards.
